var QR = (function () {
	...
	var api = {
		...
		drawCard(data, canvas, cavW, cavH) {
			wx.showLoading()
			console.log(data)
			canvas = canvas || _canvas;
			...
			//ctx.drawImage(data.logo, 250, 30, 100, 100)
			ctx.draw(true)
			...
			ctx.fillText(data.title, 300, 180)
			ctx.draw(true)
			// ctx.setFontSize(25)
			// ctx.setFillStyle('#999')
			// ctx.fillText(data.title, 300, 230)
			// ctx.draw(true)
			ctx.setFillStyle('#f4f4f4')
			ctx.fillRect(140, 290, 320, 320)
			ctx.draw(true)
			var size = Math.min(300, 300);
			var str = this.utf16to8(data.buyUrl); //增加中文显示buyUrl
			var frame = this.getFrame(str),
				px = Math.round(size / (width + 8));
			var roundedSize = px * (width + 8),
				offset = Math.floor((size - roundedSize) / 2);
			...
			for (var i = 0; i < width; i++) {
				for (var j = 0; j < width; j++) {
					if (frame[j * width + i]) {
						ctx.fillRect(px * (4 + i) + offset + 150, px * (4 + j) + offset + 300, px, px);
					}
				}
			}
			ctx.draw(true);
			...
			ctx.fillText('微信扫描二维码领取次卡', 300, 660)
			ctx.draw(true)
			drawimg(data.logo).then(img=>{
				ctx.drawImage(img, 250, 30, 100, 100)
				ctx.draw(true)
			})
			wx.hideLoading()
		},
		...
		drawPayCode(str, canvas,topimg,bottomimg) {
			wx.showLoading()
			ecclevel = ecclevel;
			canvas = canvas || _canvas;
			if (!canvas) {
