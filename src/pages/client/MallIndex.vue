<template>
  <div class="MallIndex">
    ...
    <section class="newGoods section">
      ...
      <Slick :ulWidth="(266*goodsList.length)+(10*(goodsList.length-1))" :showWidth="(266*4)+(10*3)" :height="360">
        <ul class="goodsList" :style="{width:`${(266*goodsList.length)+(10*(goodsList.length-1))}px`}" slot="list">
          <GoodsItem v-for="(item,index) in goodsList" :style="{marginRight: (index+1)%4===0?'0px':'10px'}"
            :key="+item.id" :id="item.id" :img="item.img" :name="item.name" :price="item.price" />
        </ul>
      </Slick>
    </section>
    ...
    <section class="hotGoods section">
      ...
      <div class="content">
        <ul class="left">
          <GoodsItem :id="goodsList[0].id" :img="goodsList[0].img" :name="goodsList[0].name"
            :price="goodsList[0].price" />
        </ul>
        <ul class="right">
          <GoodsItem v-for="(item,index) in goodsList" :style="{marginBottom: index<=2?'10px':'0px'}" :key="+item.id"
            :id="item.id" :img="item.img" :name="item.name" :price="item.price" />
        </ul>
      </div>
    </section>
    ...
    <section>
      <SectionHeader title="商品" tips="" moreText="查看更多>" @click.native="selectType" />
      <ul class="content">
        <!-- filterGoodsByType(item.id) -->
        <GoodsItem v-for="(item,index) in shopList"    
          :style="{marginRight: (index+1)%4===0?'0px':'25px'}" :key="+item.goodsId" :id="item.goodsId"
          :name="item.goodsName" :price="item.deliveryPrice" :goodsSpec="item.goodsSpec" :goodsUnit="item.goodsUnit" />
      </ul>
    </section>
  </div>
</template>
...
<script>
  ...
  import { getTypes, getGoodsList, institutionDetail, goodList } from '../../api/client';
  import SectionHeader from '../../components/SectionHeader';
  import ZoomImg from '../../components/ZoomImg';
  import GoodsItem from '../../components/GoodsItem';
  import Slick from '../../components/Slick';
  import FadeSwiper from '../../components/FadeSwiper';
  import { getClientSize, getScrollWidth,steamroller,distinct } from '../../util/util';
  ...
  export default {
    ...
    data() {
      return {
        typeList: [{ id: 1, name: "商品", price: 100, img: "http://yanxuan.nosdn.127.net/0266209ded1751f599fe0dc21bb33e02.jpg" }, { id: 2, name: "商品列表", price: 100, img: "http://yanxuan.nosdn.127.net/0266209ded1751f599fe0dc21bb33e02.jpg" }, { id: 3, name: "商品列表", price: 100, img: "http://yanxuan.nosdn.127.net/0266209ded1751f599fe0dc21bb33e02.jpg" }, { id: 4, name: "商品列表", price: 100, img: "" }, { id: 5, name: "商品列表", price: 100, img: "http://yanxuan.nosdn.127.net/0266209ded1751f599fe0dc21bb33e02.jpg" }],
        goodsList: [{ id: 1, name: "商品", price: 100, img: "http://yanxuan.nosdn.127.net/0266209ded1751f599fe0dc21bb33e02.jpg" }, { id: 2, name: "商品列表", price: 100, img: "http://yanxuan.nosdn.127.net/0266209ded1751f599fe0dc21bb33e02.jpg" }, { id: 3, name: "商品列表", price: 100, img: "http://yanxuan.nosdn.127.net/0266209ded1751f599fe0dc21bb33e02.jpg" }, { id: 4, name: "商品列表", price: 100, img: "" }, { id: 5, name: "商品列表", price: 100, img: "http://yanxuan.nosdn.127.net/0266209ded1751f599fe0dc21bb33e02.jpg" }],
        shopList:[],
        initTimestamp: 0,
        newTimestamp: 0,
        timer: null,
    ...
    methods: {
      ...
      selectType(typeId) {
        ...
        this.navTo('/mall/show/goodsList/' + typeId);
      },
      getGoodsList(typeId) {
      ...
      // handleScroll(e){
      //  	var scrollBottom = document.documentElement.scrollBottom || document.body.scrolBottom
      //  	console.log(scrollBottom,"滚动条事件" )
      // }
    },
    ...
    created() {
      ...
      // goodList({ appId: this.clientId, merchantCode: this.clientCode, currentPage: 1, pageSize: 20 }).then(res => {
      //   console.log(res, "商品列表")
      // })
      // institutionDetail({ appId: this.clientId, merchantCode: this.clientCode }).then(res => {
      //   // console.log(res.result.merchantGroupList,JSON.parse(res.result.merchantGroupList[1].groupInfo), "机构详情")
      //   let arr = []
      //   res.result.merchantGroupList.map((item,index)=>{
      //     // console.log(JSON.parse(item.merchantCodes))
      //     arr.push(JSON.parse(item.merchantCodes))
      //   })
      //   // console.log(arr)
      //   // console.log(distinct(steamroller(arr)).toString())
      //   goodList({ appId: this.clientId, merchantCodes: distinct(steamroller(arr)).toString(), currentPage: 1, pageSize: 20 }).then(res => {
      //   console.log(res, "商品列表")
      //   this.shopList=res.result.items
      // })
      // })
    },
