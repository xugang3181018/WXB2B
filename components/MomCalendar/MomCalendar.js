...
Component({
  ...
  methods: {
    ...
    selectDay(e) {
      ...
      let startDateInfo = this.startDate(start);
      if (moment(startDateInfo).isAfter(this.data.taday)) {
        wx.showToast({
          title: '起始时间不能超过当日时间',
          icon: 'none'
        })
    ...
    // 判断是否是最新月份
    isEqualMon() {
      let {taday,title} = this.data;
      let tadays = taday.slice(0,7);
      if(moment(title).isSame(tadays)) {
        this.setData({
          nextDateIcon: false
        })
      } else {
